...
def proxy_handler(client_socket, remote_host, remote_port, receive_first):
    ...
    if receive_first:
        ...
        if len(remote_buffer):
            print "[<==] Sending %d bytes to our localhost." % len(remote_buffer)
            client_socket.send(remote_buffer)

    # now lets loop and read from local, send to remote, send to local
    while True:

        # read from local hosr
        ...
        if len(local_buffer):
            ...
            # send it to out request handler
...
# modift any requests destined for the remote host
def request_handler(buffer):
    # perform packet modifications
    return buffer

# modify and responses destined to the local host
...
def main():
    ...
    # now spin up our listenting socket
